function dy = fm_model_51(p, ndata)

% This model makes a loop using two leaves:
%
%	The x's and probabilities are fixed so that leaves have identical input
%
...
p_north = [p(1) p(2) p(3) p(4) p(5)  p(6)   0.5]; % as in a generic leaf
p_south = [p(1) p(2) p(7) p(8) p(9) p(10)  0.5]; % p(1), p(2) and p(7) are fixed
w_weight = 0.5;
